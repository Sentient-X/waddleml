...
| `_sync.py` | `SyncEngine` — background upload of the DuckDB spool to the hosted platform (idempotent batches, persisted outbox, artifact uploads); active only when `WADDLE_API_URL`/`SX_API_KEY` are set |
