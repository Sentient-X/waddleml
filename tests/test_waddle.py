...
def test_upsert_and_get_repo(tmp_path: Path):
    ...
    repo_info = db.upsert_repo(
        "main", str(repo_path), "https://example.com/repo.git", default_branch="dev"
    )
...
def test_record_commit(tmp_path: Path):
    ...
    row = db.fetchone(
        "SELECT commit_sha, author FROM commits WHERE commit_sha = $1", [sha]
    )
