...
def test_metrics_carry_worker_and_lineage(tmp_path, monkeypatch):
    ...
    worker = WorkerInfo(rank=3, local_rank=1, world_size=8, node_id="trainer-b", attempt=2)
...
def test_init_observes_dirty_git_tree_without_committing(tmp_path, monkeypatch):
    ...
    before = subprocess.check_output(["git", "rev-parse", "HEAD"], cwd=tmp_path, text=True)
    monkeypatch.chdir(tmp_path)

    run = waddle.init(system_metrics=False)
    run.finish()

    after = subprocess.check_output(["git", "rev-parse", "HEAD"], cwd=tmp_path, text=True)
    assert after == before
    conn = duckdb.connect(str(tmp_path / ".waddle" / "waddle.duckdb"))
    digest = conn.execute("SELECT lineage->>'source_patch_sha256' FROM runs").fetchone()[0]
