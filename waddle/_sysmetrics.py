...
class SystemMonitor:
    def __init__(self, run: Run, interval: float = 5.0):
        ...
        self._step = 0
        self._prev_io: tuple | None = (
            None  # (ts, disk_io_counters, net_io_counters) for rate deltas
        )
    ...
    def _sample(self) -> Dict[str, float]:
        ...
        if self._has_psutil:
            try:
                ...
                metrics["system/proc_memory_gb"] = (
                    psutil.Process().memory_info().rss / (1024**3)
                )
                now = time.time()
                disk = psutil.disk_io_counters()
                net = psutil.net_io_counters()
                # Both are None on platforms without the counters; skip I/O rates there.
                if disk is not None and net is not None:
                    if self._prev_io is not None:
                        p_ts, p_disk, p_net = self._prev_io
                        dt = max(1e-9, now - p_ts)
                        mb = 1024**2
                        metrics["system/disk_read_mbps"] = (
                            (disk.read_bytes - p_disk.read_bytes) / mb / dt
                        )
                        metrics["system/disk_write_mbps"] = (
                            (disk.write_bytes - p_disk.write_bytes) / mb / dt
                        )
                        metrics["system/net_sent_mbps"] = (
                            (net.bytes_sent - p_net.bytes_sent) / mb / dt
                        )
                        metrics["system/net_recv_mbps"] = (
                            (net.bytes_recv - p_net.bytes_recv) / mb / dt
                        )
        ...
        if self._has_pynvml:
            try:
                ...
                for i in range(self._gpu_count):
                    ...
                    used = float(mem_info.used)
                    metrics[f"{prefix}_memory_used_gb"] = used / (1024**3)
                    total = max(1.0, float(mem_info.total))
                    metrics[f"{prefix}_memory_percent"] = 100.0 * used / total
                    try:
